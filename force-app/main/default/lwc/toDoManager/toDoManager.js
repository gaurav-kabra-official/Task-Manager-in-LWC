 ...
 * @author Gaurav Kabra (Inspired by ManishChodhary8585)
 */
...
export default class toDoManager extends LightningElement {
  ...
  // before spring'20 , private properties (primitives and objects) used to be non-reactive i.e. didn't show changes in page even if updated
  // so recommended to use @track
  @track time = "10:10:01 PM";
  ...
  connectedCallback() {
    ...
    //get time periodically after every second
    setInterval(() => {
      this.getTime();
    }, 1000);
  }
  ...
  getTime() {
    ...
    this.time = `${this.getHour(hour)}:${this.getDoubleDigitMin(min)}:${this.getDoubleDigitSec(sec)} ${this.getPMAM(hour)}`;
  ...
  //convert single digit to double digit E.g. 8:9 -> 8:09
  getDoubleDigitMin(digit) {
    return digit < 10 ? "0" + digit : digit;
  }

  getDoubleDigitSec(digit){
    return digit < 10? "0" + digit : digit;
  }
